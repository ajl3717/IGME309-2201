...
bool Simplex::MyOctant::IsColliding(uint a_uRBIndex)
{
	...
	//As the Octree will never rotate or scale, use Axis Allgined Bounding Box
...
{
	//If this node reaches the maximum depth without changes
...
{
	//Recursively remove all the children in the tree
	for (uint i = 0; i < m_uChildren; i++)
	{
...
{
	//Don't apply anywhere but the root
	if (m_uLevel != 0)
	{
		return;
	}
